...
export const usePagination = ({ pageCount, currentPage }: UsePaginationProps) => {
  ...
  const items: PaginationItem[] = useMemo(() => {
    ...
    if (pageCount <= maxVisibleBlocks) {
      paginationItems = ['previous', ...range(1, pageCount), 'next']
    } else {
      ...
      if (currentPage <= visibleMiddle) {
        paginationItems = ['previous', ...range(1, visibleMiddle + 1), 'ellipsis', pageCount, 'next']
        ...
        paginationItems = ['previous', 1, 'ellipsis', ...range(pageCount - visibleMiddle, pageCount), 'next']
