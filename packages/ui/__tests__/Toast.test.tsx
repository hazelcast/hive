...
describe('Toast', () => {
  ...
  it.each(toastBasicTestData)(
    'Renders %s Toast with correct icon and content',
    async (type, { icon, ariaLabel }) => {
      ...
      expect(AlertElement.findDataTest('toast-content').text()).toBe(
        content,
      )

      expect(
        wrapper.findDataTest('toast-icon').props(),
      ).toMatchObject({
        icon,
        ariaLabel,
      })
