...
export const Sizes = () => {
  const [a, setA] = useState<number | undefined>(8)
  const [b, setB] = useState<number | undefined>(42)
  return (
    <div className={s.section}>
      <Caption>
        Two sizes: <strong>small</strong> for dense tables and toolbars, and <strong>medium</strong> (default) for most forms.
      </Caption>
      <div className={`${s.grid} ${s.gridSizes}`}>
        <Cell label="Small">
          <NumberField name="size-small" size="small" label="Favorite number" value={a} onChange={setA} />
        </Cell>
        <Cell label="Medium">
          <NumberField name="size-medium" label="Favorite number" value={b} onChange={setB} />
        </Cell>
      </div>
    </div>
  )
